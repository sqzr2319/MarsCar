...
arm_servo_1_init=1450
arm_servo_2_init=1800
arm_servo_3_init=800
arm_servo_4_init=1800
...
#2.5 定义车速
car_run_speed= 1000   				#小车直行运动速度，范围0~1000us 此数值是PWM输出数值，范围为500us-2500us之间，其中1500us电机停止，大于1500us正转，小于1500us反转
car_run_time = 5000					#小车直行时间1000=1s	该数值用于启动车子的运动

#小车转弯参数
car_turn_speed = 400					#小车转弯时的速度
car_turn_angle = 500 					#小车转弯角度，范围0~1000
...
def car_servos_init():
    ...
    print("Car servos are initializing")
    uart.uart_send_str(Srt)
    time.sleep_ms(1000)  # 等待底盘舵机初始化完成
...
def car_turn(turn_angle,turn_time):
    Srt='#011P{0:0>4d}T{4:0>4d}!#012P{1:0>4d}T{4:0>4d}!#013P{2:0>4d}T{4:0>4d}!#014P{3:0>4d}T{4:0>4d}!'.format(car_servo_fl_init-turn_angle,car_servo_fr_init-turn_angle,car_servo_bl_init+turn_angle,car_servo_br_init+turn_angle,turn_time)
    uart.uart_send_str(Srt)
        
#3.4.4 小车运动+转向
def car_run_and_turn(run_speed,turn_angle,run_time):
    Srt='#001P{0:0>4d}T{8:0>4d}!#002P{1:0>4d}T{8:0>4d}!#003P{2:0>4d}T{8:0>4d}!#004P{3:0>4d}T{8:0>4d}!#011P{4:0>4d}T{8:0>4d}!#012P{5:0>4d}T{8:0>4d}!#013P{6:0>4d}T{8:0>4d}!#014P{7:0>4d}T{8:0>4d}!'.format(1500-run_speed,1500+run_speed,1500-run_speed,1500+run_speed,car_servo_fl_init-turn_angle,car_servo_fr_init-turn_angle,car_servo_bl_init+turn_angle,car_servo_br_init+turn_angle,run_time)
    print(f"Car running with speed: {run_speed}, turn angle: {turn_angle}")
    uart.uart_send_str(Srt)
...
def arm_servos_init():
    ...
    Srt = '#021P{0:0>4d}T{4:0>4d}!#022P{1:0>4d}T{4:0>4d}!#023P{2:0>4d}T{4:0>4d}!#024P{3:0>4d}T{4:0>4d}!'.format(arm_servo_1_init,arm_servo_2_init,arm_servo_3_init,arm_servo_4_init,1500)
    arm_servo_1_pwm = arm_servo_1_init
    arm_servo_2_pwm = arm_servo_2_init
    arm_servo_3_pwm = arm_servo_3_init
    arm_servo_4_pwm = arm_servo_4_init
    print("Arm servos are initializing")
    uart.uart_send_str(Srt)
    time.sleep_ms(1500)  # 等待机械臂舵机初始化完成
...
def arm_move_1(arm_id,arm_ang,move_time):
    ...
    if arm_id == arm_servo_1:
        arm_servo_1_pwm = arm_ang
        print(f"Arm 1 running with angle: {arm_servo_1_pwm}")
    elif arm_id == arm_servo_2:
        arm_servo_2_pwm = arm_ang
        print(f"Arm 2 running with angle: {arm_servo_2_pwm}")
    elif arm_id == arm_servo_3:
        arm_servo_3_pwm = arm_ang
        print(f"Arm 3 running with angle: {arm_servo_3_pwm}")
    elif arm_id == arm_servo_4:
        arm_servo_4_pwm = arm_ang
        print(f"Arm 4 running with angle: {arm_servo_4_pwm}")
    uart.uart_send_str(armSrt)
    time.sleep_ms(move_time)  # 等待机械臂运动完成
...
def arm_move_4(arm_ang1,arm_ang2,arm_ang3,arm_ang4,move_time):
    ...
    arm_servo_1_pwm = arm_ang1
    arm_servo_2_pwm = arm_ang2
    arm_servo_3_pwm = arm_ang3
    arm_servo_4_pwm = arm_ang4
    print(f"Arm running with angle: 1:{arm_servo_1_pwm},2:{arm_servo_2_pwm},3:{arm_servo_3_pwm},4:{arm_servo_4_pwm}")
    uart.uart_send_str(armSrt)
    time.sleep_ms(move_time)  # 等待机械臂运动完成
...
def funcspeed(normalized_speed,normalized_angle):
    if abs(normalized_angle)>0.9:
        if normalized_speed>=0:
            normalized_speed=normalized_speed*0.4+0.6
        else:
            normalized_speed=normalized_speed*0.4-0.6
    return int(normalized_speed* car_run_speed)

def funcangle(normalized_angle,normalized_speed):
    if 1>=normalized_angle>0.9:
        normalized_angle=normalized_angle*0.4+(1-abs(normalized_speed))*0.6
    elif -1<=normalized_angle< -0.9:
        normalized_angle=normalized_angle*0.4-(1-abs(normalized_speed))*0.6
    else:
        normalized_angle=normalized_angle*0.4
    return int(normalized_angle * car_turn_angle)
...
def loop_ps2(): 
    ...
    if ps2.ButtonPressed('L1'):
        value_left_x = ps2.Analog(7)  # 左摇杆X轴
        value_left_y = ps2.Analog(8)  # 左摇杆Y轴
        print(f"Left X Joystick: {value_left_x}, Left Y Joystick: {value_left_y}")
        # 计算车速
        normalized_speed = (128 - value_left_y) / 128
        normalized_angle= (128 - value_left_x) / 128
        speed_pwm = funcspeed(normalized_speed,normalized_angle)
        turn_angle = funcangle(normalized_angle,normalized_speed)
        # 控制小车运动
        car_run_and_turn(speed_pwm, turn_angle, ps2_arm_inc)
        
    if ps2.ButtonPressed('L3'):
        car_servos_init()
        print("L3 button pressed. ")
            
    # 通过右摇杆模拟值控制机械臂运动
    if ps2.ButtonPressed('L2'):
        value_x=ps2.Analog(5)  # 右摇杆X轴
        turn_angle= int((128 - value_x) / 128 * 30)  # 转向角度
        arm_move_1(arm_servo_1,arm_servo_1_pwm+turn_angle,ps2_arm_inc)
    if ps2.ButtonPressed('R1'):
        value_y=ps2.Analog(6) # 右摇杆Y轴
        turn_angle= int((128 - value_y) / 128 * 30)  # 转向角度
        arm_move_1(arm_servo_2,arm_servo_2_pwm-turn_angle,ps2_arm_inc)
    if ps2.ButtonPressed('R2'):
        value_y=ps2.Analog(6)  # 右摇杆Y轴
        turn_angle= int((128 - value_y) / 128 * 30)  # 转向角度
        arm_move_1(arm_servo_3,arm_servo_3_pwm-turn_angle,ps2_arm_inc)
    if ps2.ButtonPressed('R3'):
        arm_servos_init()  # 重置机械臂舵机到初始位置
    if ps2.ButtonPressed('PAD_UP'):
        turn_angle=30
        arm_move_1(arm_servo_4,arm_servo_4_pwm+turn_angle,ps2_arm_inc)
    ...
    if ps2.ButtonPressed('CROSS'):
        arm_move_4(arm_servo_1_pwm,1020,arm_servo_3_pwm,1050,ps2_arm_inc*10)
        arm_move_1(arm_servo_3,950,ps2_arm_inc*3)
    if ps2.ButtonPressed('TRIANGLE'):
        arm_move_4(arm_servo_1_pwm,650,1200,1800,ps2_arm_inc*10)
        arm_move_4(arm_servo_1_pwm,1500,860,2300,ps2_arm_inc*10)
        angle=random.randint(2440,2480)
        arm_move_1(arm_servo_1,angle,ps2_arm_inc*20)
        arm_move_1(arm_servo_2,1350,ps2_arm_inc*8)
        arm_move_4(arm_servo_1_pwm,1100,1500,arm_servo_4_pwm,ps2_arm_inc*8)
        arm_move_4(arm_servo_1_pwm,980,2070,1780,ps2_arm_inc*8)
        time.sleep(1)
        arm_servos_init()
